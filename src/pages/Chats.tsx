...
import { Send, Paperclip, Check, FileText, Image, Search, ChevronLeft, Phone, MessageSquare } from 'lucide-react'
...
const INITIAL_CHATS: ChatRoom[] = [
  {
    ...
    partnerName: '김차주',
    partnerType: 'driver',
    vehicleNo: '경북 80사 1234',
    phone: '010-4422-9988',
    unreadCount: 2,
    lastUpdated: new Date(Date.now() - 5 * 60 * 1000).toISOString(),
    messages: [
      { id: 'm1', sender: 'dispatcher', text: '김차주 차주님, 상차 완료되셨나요?', timestamp: new Date(Date.now() - 60 * 60 * 1000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) },
      { id: 'm2', sender: 'partner', text: '네, 방금 상차 마치고 출발했습니다. 오후 3시쯤 하차지 도착 예정입니다.', timestamp: new Date(Date.now() - 45 * 60 * 1000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) },
      { id: 'm3', sender: 'dispatcher', text: '확인 고맙습니다. 하차지 도착하시면 인수증 촬영해서 보내주세요.', timestamp: new Date(Date.now() - 30 * 60 * 1000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) },
      { id: 'm4', sender: 'partner', text: '알겠습니다. 도착해서 사진 보내드리겠습니다.', timestamp: new Date(Date.now() - 10 * 60 * 1000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }
    ]
  },
  {
    id: 'room_client_1',
    partnerName: '가온물류 (박대리)',
    partnerType: 'client',
    phone: '02-8877-2233',
    unreadCount: 0,
        ...
        {activeRoom ? (
          <>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', color: 'var(--text-tertiary)', fontSize: '0.7rem', marginTop: '0.15rem' }}>
                    <Phone size={10} /> {activeRoom.phone}
                  </div>
                </div>
              </div>
            </div>
                  ...
                  >
                    ...
                    <div style={{
                      ...
                      {msg.file && (
                        <div style={{
                          ...
                          {msg.file.type === 'photo' ? (
                            <Image size={24} style={{ color: isMyMsg ? '#ffffff' : 'var(--primary)' }} />
                          ) : (
                            <FileText size={24} style={{ color: isMyMsg ? '#ffffff' : 'var(--success)' }} />
                          )}
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{
                              fontSize: '0.76rem',
                            ...
                            <div style={{
                              fontSize: '0.62rem',
                              color: isMyMsg ? 'rgba(255, 255, 255, 0.7)' : 'var(--text-tertiary)',
                              textTransform: 'uppercase'
                            }}>
                              {msg.file.type === 'receipt' ? '인수증' : msg.file.type === 'invoice' ? '세금계산서' : '현장사진'}
                            </div>
                          </div>
                        </div>
                      )}
                    ...
                    {isMyMsg && (
                      <div style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.1rem', color: 'var(--primary)' }}>
                          <Check size={10} /> 읽음
                        </div>
            ...
            <div style={{
              ...
              {showFileMenu && (
                <>
                  <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 90 }} onClick={() => setShowFileMenu(false)} />
                  <div style={{
                    position: 'absolute',
                    bottom: '100%',
                    left: '1.25rem',
...
<form
              ...
              >
                ...
                <button
                  ...
                  style={{
                    backgroundColor: showTemplates ? 'var(--primary-light)' : 'transparent',
                    border: '1px solid var(--border-color)',
                    borderRadius: 'var(--radius-sm)',
                    padding: '0.45rem 0.65rem',
                    fontSize: '0.74rem',
                    fontWeight: 700,
                    cursor: 'pointer',
                    color: showTemplates ? 'var(--primary)' : 'var(--text-secondary)',
                    transition: 'all var(--transition-fast)',
                    display: 'flex',
                    alignItems: 'center',
                    gap: '0.2rem',
